...
if __name__ == "__main__":
    ...
    connected_verticies = [[vertex1, vertex2]]
    while len(connected_verticies[0]) < len(box_coordinates):
        vertex1, vertex2 = distance_dictionary[min(distance_dictionary.keys())][0]
        distance_dictionary.pop(min(distance_dictionary.keys()))
        added = False
    ...
    print(f"{len(connected_verticies[0])} of {len(box_coordinates)} boxes connected")
    print(box_coordinates[vertex1], box_coordinates[vertex2])
    print(box_coordinates[vertex1][0]*box_coordinates[vertex2][0])